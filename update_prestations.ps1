# PowerShell script to update all prestation pages

$basePath = "c:\Users\Aba Baker\StoonEvents\resources\js\Pages\Prestations"

# Define all prestations with their data
$prestations = @(
    @{
...
"@
    ...
    @{
        ...
        content = @"
                            ...
                            <h3 className="text-2xl font-bold text-gray-800 mt-8 mb-4">Nos attractions :</h3>
                            <ul className="space-y-3 text-lg text-gray-700">
                                <li>🦆 Pêche aux canards</li>
                                <li>🎯 Chamboule-tout</li>
                                <li>💍 Jeu des anneaux</li>
                                <li>🏹 Tir à l'arc</li>
                                <li>🎳 Jeu de quilles</li>
                                <li>🎨 Ateliers de coloriage</li>
                                <li>💄 Maquilleuse professionnelle</li>
                            </ul>
                            
                            <h3 className="text-2xl font-bold text-gray-800 mt-8 mb-4">Stands gourmands :</h3>
                            <ul className="space-y-3 text-lg text-gray-700">
                                <li>🥞 Crêpes</li>
                                <li>🍭 Barbe à papa</li>
                                <li>🍿 Pop-corn</li>
...
"@
        cta = "Prêt à Organiser votre Kermesse ?"
        ctaDesc = "Contactez-nous pour créer une kermesse festive et mémorable"
    },
    @{
...
export default function {COMPONENT_NAME}() {
    const images = [{IMAGE_RANGE}];

    return (
        <>
            <Head title="{TITLE} - Stoon Events" />
            <Navbar />
            <SocialSidebar />
            <WhatsAppButton />
            ...
            <div className="min-h-screen bg-gray-50">
                ...
                <section className="relative h-[60vh] md:h-[70vh] overflow-hidden">
                    <img 
                        src="/assets/images/nosPrestations/{FOLDER}/1.jpeg" 
                        alt="{TITLE}" 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {images.map((num) => (
                                <div key={num} className="relative overflow-hidden rounded-lg shadow-lg group">
                                    <img 
                                        src={`/assets/images/nosPrestations/{FOLDER}/${{num}}.jpeg`}
                                        alt={`{TITLE} ${{num}}`}
...
Write-Output "Processing $($prestations.Count) files..."

foreach ($prestation in $prestations) {
    Write-Output "Processing $($prestation.file)..."
    
    $componentName = $prestation.file -replace '\.jsx$', ''
    $imageRange = (1..$prestation.images) -join ', '
    
    $content = $template `
        -replace '\{COMPONENT_NAME\}', $componentName `
        -replace '\{TITLE\}', $prestation.title `
        -replace '\{EMOJI\}', $prestation.emoji `
        -replace '\{FOLDER\}', $prestation.folder `
        -replace '\{IMAGE_RANGE\}', $imageRange `
        -replace '\{CONTENT\}', $prestation.content `
        -replace '\{CTA\}', $prestation.cta `
        -replace '\{CTA_DESC\}', $prestation.ctaDesc
    
    $filePath = Join-Path $basePath $prestation.file
    Set-Content -Path $filePath -Value $content -Encoding UTF8
    Write-Output "  ✓ Updated $($prestation.file)"
}

Write-Output ""
Write-Output "All files updated successfully!"
